...
if (document.getElementById("heading").classList.contains("title")) {
    document.querySelector(".title").textContent = 'Cloud Droid'
}
...
const clouddroidlink = videolink.replace("/watch/", "/");

function vlc_player() {
    const openclouddroidlink = clouddroidlink;
...
function playit_player() {
    const openbisallink = clouddroidlink;
...
function clouddroidDownload() {
    const openbisallink = clouddroidlink;
    window.location.href = openclouddroidlink;
}
